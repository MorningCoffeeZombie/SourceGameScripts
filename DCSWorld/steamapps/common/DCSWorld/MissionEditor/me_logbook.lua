        ...
        if (isUserEvent(event,playerTable.playerUnitId)) then			
            ...
            if (not isTrack) or controlTaken then
                ...
                if (event.type == 'kill' and event.initiatorMissionID ~= nil and playerTable.playerUnitId == event.initiatorMissionID) then				
					...
					if event.initiatorMissionID ~= event.targetMissionID then  -- MY EDITS  
						...
						if unitTarget then
							local country_idTarget = unitTarget.boss.boss.id  -- MY EDITS  
							local victim = unitTarget  -- MY EDITS  
							local victimType = victim.boss.type  -- MY EDITS  
